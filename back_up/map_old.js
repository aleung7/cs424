var state_svg = d3.select("div#Trade_App")
				.append("svg")
				.attr("id", "states")
...
var div_country = d3.select("div#Trade_App").append("div").attr("class","tooltip").attr("class", "label").style("opacity",0)

var country_svg = d3.select("div#Trade_App").append("svg").attr("id", "countries")

country_svg.attr("width", 600)
				.attr("height", 700)
				
d3.csv("import_state.csv", function(import_trade){		
	d3.csv("country_export.csv", function(data){
		var trades = {};
		var numbers = [];
	...
	// 												.style("top", "500px")
							...
							country_svg.selectAll("path.countries")
								...
								.on("mouseover", function(d){
									div_country.transition().duration(500).style("opacity", 9)
									for(var i = 0; i < data.length; i++){
										if(d3.select("path.state." + data[i]["statename"]))
											if(d3.select("path.state."+ data[i]["statename"].toLowerCase()).attr("fill") === "#008000"){
												if(d3.select("path.country." + d.properties.name.toLowerCase()).attr("fill") === "#008000"){
													div_country.html(data[i]["countryd"] + ": " + data[i]["val2012"]);
													//break;
												}
											}
